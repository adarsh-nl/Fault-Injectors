...
------------------------------
For a camera-based BEV model, ``K`` and ``T_cam_to_agent`` are **tensors on
the model path**, not preprocessing metadata. Every such model in this
repository places image content on the ground plane by projecting through
them, and the mechanism differs without changing that fact:

    CoBEVT      lifts by matching unit ray directions computed from K and E
    Where2comm  splats a predicted depth distribution along rays built from
                K and E

Change those matrices and image content lands somewhere else on the BEV grid,
with the pixels themselves untouched. Nothing else in this package perturbs
them, so a camera fault benchmark without this injector could corrupt every
input to the model *except* the geometry the model is built around.
...
that no alarm fires. That is precisely the silent-degradation regime a
robustness benchmark exists to measure.
...
It lived in ``cobevtbench`` while there was one consumer, with a note that a
second camera paper would be the trigger to promote it. ``w2cbench`` is that
second paper. It sits here now, beside every other physical sensor
corruption, and ``cobevtbench.faults.calibration`` re-exports.
"""

"""EXACT MultiCorrupt / Hahner LiDAR snow machinery (arXiv:2402.11677), extracted verbatim.

Source: github.com/ika-rwth-aachen/MultiCorrupt  converter/lidar.py, which adapted
Hahner et al., "LiDAR Snowfall Simulation" (CVPR 2022, github.com/SysCV/LiDAR_snow_sim).

Everything between the BEGIN/END VERBATIM markers is copied unchanged:
compute_occupancy, snowfall_rate_to_rainfall_rate, ransac_polyfit,
estimate_laser_parameters, process_single_channel, binary_angle_search,
compute_occlusion_dict, tangent_angles_to_interval_angles,
do_angles_intersect_particles, angles_to_lines, tangent_lines_to_tangent_angles,
tangents_from_origin, distances_of_points_to_lines, get_occlusions,
received_power, xsi.

`simulate_snow_griffin` at the bottom is MultiCorrupt's `simulate_snow` with EXACTLY
ONE substitution, marked inline: the nuScenes-lidarseg ground lookup (unavailable on
Griffin) is replaced by a caller-supplied boolean `ground` mask. Every other line of
the function body is unchanged.

Runtime data files expected next to this module:
  snow_laser.yaml   (HDL-64 sensor calibration; copied from MultiCorrupt)

One marked NumPy-2.x compatibility patch exists inside estimate_laser_parameters
(legacy list-of-array indexing; identical bin midpoints, see inline comment).
  npy/gunn_*.npy    (snowflake particle files; generated with Hahner's exact
                     dart_throwing sampler, see snowflake_sampling.py)
"""
import os
import copy
...
def estimate_laser_parameters(pointcloud_planes, calculated_indicent_angle, power_factor=15, noise_floor=0.7,
    ...
    # --- NumPy 2.x compatibility (semantics-preserving; upstream lines kept below) ---
    # Upstream indexed xedges with a list containing an array (idx1 = [i + 1 for i in idx]);
    # legacy NumPy treated that as tuple indexing (1-D result), NumPy 2.x treats it as
    # advanced indexing (2-D result), which breaks the linregress fit. The tuple form
    # below computes the identical bin midpoints.
    #     idx1 = [i + 1 for i in idx]
    #     x = (xedges[idx] + xedges[idx1]) / 2
    x = (xedges[idx[0]] + xedges[idx[0] + 1]) / 2
...
def simulate_snow_griffin(pc: np.ndarray,
                  ground: np.ndarray,
                  severity: int,
    ...
    # --- GRIFFIN SUBSTITUTION (the only change vs MultiCorrupt simulate_snow) ---
    # Upstream derives `ground` from nuScenes lidarseg labels via learning_map
    # (driveable_surface==11 | other_flat==12 | sidewalk==13). Griffin has no
    # lidarseg, so the boolean ground mask is supplied by the caller instead
    # (geometric ground estimate; see lidar_snow.py).
    ground = np.asarray(ground, dtype=bool).reshape(-1)
    # --- END SUBSTITUTION ---

...
system profiling, communication volume, tap dumps. Running that between epochs
would cost more than the epoch.

The validator answers one question -- is this checkpoint better than the last?
-- and returns a ``score`` the trainer selects on. It shares the model's
``eval()`` requirement with the tester for the same reason: A17 makes the
selector stochastic in training mode, so anything measured there is a draw from
the bandwidth curriculum rather than a property of the model.
"""
...
class Validator:
    ...
    reads. The alias exists so the trainer never learns which metric a paper
    selects on.
    ...
    def run(self, model) -> Dict[str, float]:
        ...
        The mode is restored because ``fit`` calls this mid-loop: leaving the
        model in ``eval()`` would silently switch off the bandwidth curriculum
        and every BatchNorm update for the remainder of training.
        """

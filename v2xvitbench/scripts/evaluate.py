...
Score a checkpoint under exactly one condition.
    ...
    python -m v2xvitbench.scripts.evaluate --checkpoint best.pt
    python -m v2xvitbench.scripts.evaluate --checkpoint best.pt faults=type_flip
    python -m v2xvitbench.scripts.evaluate --checkpoint best.pt \\
        faults=delay_encoding --condition lat0.3_dly-zero

This is the benchmark machinery with the sweep narrowed to one entry, not a
second code path -- a single-condition number produced by different code
than the sweep's would be a number nobody could compare.

Without ``--condition`` the config's own top-level fault block is used as
the one condition, which is what makes ``faults=type_flip`` alone mean "the
...
from v2xvitbench.evaluation.sweeps import expand_sweep, has_fault
from v2xvitbench.scripts import _cli, common

logger = logging.getLogger("v2xvitbench.evaluate")
...
def run(cfg, checkpoint: Optional[str] = None,
    ...
    if condition is not None:
        ...
        # The group's own top-level block, minus its sweep: "this fault at
        # its default magnitude".
        top = {k: v for k, v in faults.items() if k not in ("sweep", "name")}

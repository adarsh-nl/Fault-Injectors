...
belongs to -- it steps, clips, logs and checkpoints. Everything
paper-specific is in the closure it was handed, which is what keeps it
testable with a two-parameter stub.

Taps are off during training, deliberately: their cost is small but
non-zero, and they produce nothing a training run consumes. The intermediate
tensors are the *product* only at evaluation time, where the tester threads
them through every forward so a fault run's dumps line up frame-for-frame
with the clean run's.

The metadata fault bridge is likewise absent by construction -- it is an
evaluation-time object (see ``faults/metadata.py``), so a benchmarked model
was never fitted to its own fault distribution.
"""
...
class TrainerConfig:
    ...
    Attributes
    ----------
    epochs        passes over the training set (reference: 60).
    grad_clip     max gradient norm; None disables clipping.
    amp           mixed precision, forward and backward only. Evaluation
                  runs in fp32 regardless: HMSA's masked softmax uses the
                  dtype's ``finfo.min`` as the mask value, and comparing
                  attention statistics across runs that masked at different
                  floors would add a variable no experiment asked for.
...
class Trainer:
    ...
    Purpose
        One place that owns the optimisation loop, so the paper package's
    ...
    def _record(self, epoch: int, batch_index: int,
        ...
        return TrainRecord(
            ...
            loss_align=0.0,      # no auxiliary term in this paper's objective

...
A ResNet run to completion, returning three intermediate feature maps rather
than a classification vector. SinBEVT consumes them coarse-to-fine, lifting
each onto a progressively smaller BEV grid.

Which layers, and the paper's off-by-one
----------------------------------------
The released config uses ``id_pick: [1, 2, 3]``, selecting ResNet **layer2,
layer3, layer4** -- 128, 256 and 512 channels. The paper's Appendix C names
layer1/2/3 while quoting those same channel counts, which cannot both be
true: 128/256/512 are layer2/3/4's widths in ResNet34. The code is right and
the paper's layer names are off by one. Recorded as assumption A2 and left
configurable, so the paper's literal reading remains reachable.
"""
